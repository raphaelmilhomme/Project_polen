...
def personalized_advice(pollen_levels: dict, sensitivity: str) -> tuple[str, str]:
    """
    Generate tailored advice based on the actual allergies selected,
    their individual risk levels, and the user's sensitivity setting.
 
    pollen_levels: dict of {pollen_name: level_string}
    sensitivity: "Low", "Medium", or "High"
    """
    # Find the worst level across selected pollens
    worst_level = "none"
    worst_pollens = []
    ...
    # Pollens that are at moderate or above
    ...
    # Build personalized go-outside recommendation
    if all_clear:
        if sensitivity == "High":
...
# Build per-pollen level dict (with sensitivity applied)
pollen_levels = {
...
# Determine which st call to use for go_out box based on worst level
